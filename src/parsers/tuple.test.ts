...
describe('tuple', () => {
  ...
  describe('encode', () => {
    it('encodes a static tuple', () => {
      expect(
        toHex(
          tuple.encode({ type: '(uint256, uint256)', value: [12n, 34n], buffer: new Uint8Array() }),
        ),
      ).toBe(
    ...
    it('encodes a dynamic tuple', () => {
      expect(
        toHex(
          tuple.encode({ type: '(uint256, bytes)', value: [12n, 'ab'], buffer: new Uint8Array() }),
        ),
      ).toBe(
  ...
  describe('decode', () => {
    it('decodes an encoded static tuple', () => {
      ...
      expect(tuple.decode({ type: '(uint256, uint256)', value, skip: jest.fn() })).toStrictEqual([
        12n,
        34n,
      ]);
    });
